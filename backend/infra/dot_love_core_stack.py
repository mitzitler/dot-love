...
#     * NiceGuy message parser Service
...
class DotLoveCoreStack(Stack):
    def __init__(
        ...
        **kwargs,
        ...
        # Create NiceGuy to parse messages before sending them to services
        self.dot_love_nice_guy_lambda = self.create_nice_guy_lambda()
    ...
    # NiceGuy is responsible for parsing and reformatting initial payloads from the APIGW
    def create_nice_guy_lambda(self):
        nice_guy_role = iam.Role(
            scope=self,
            id=f"{self.stack_env}-dot-love-nice-guy-lambda-role",
            assumed_by=iam.ServicePrincipal("lambda.amazonaws.com"),
        ...
        nice_guy_lambda = lambdaFx.Function(
            scope=self,
            id=f"{self.stack_env}-dot-love-nice-guy-lambda",
            runtime=lambdaFx.Runtime.PYTHON_3_11,
            handler="index.handler",
            role=nice_guy_role,
            code=lambdaFx.Code.from_asset("infra/lambda/nice_guy/"),
        ...
        # Create Request Authorizer lambda
        self.lambda_authorizer = HttpLambdaAuthorizer(
            f"{self.stack_env}-dot-love-nice-guy-lambda-authorizer",
            nice_guy_lambda,
            response_types=[HttpLambdaResponseType.SIMPLE],
        )

        return {"function": nice_guy_lambda, "role": nice_guy_role}

    ###################################################
    # DOT LOVE API GATEWAY ORCHESTRATION
    ###################################################
    def create_dot_love_api_gw(self):
    ...
    def create_global_dependency_layer(self):
        ...
        #   pip3 install -r ~/code/dot-love/backend/infra/lambda/nice_guy/requirements.txt --target ~/code/dot-love/backend/infra/lambda/nice_guy/layer/python/lib/python3.7/site-packages
